...
新建一个类实现CustomLogExceptionHandler接口，重写handleException，然后在这个方法内对异常进行处理，返回的出参ExceptionVo中的isShowThrowable表示是否将异常记录到日志throwable字段，responseParams则是一个存储出参的map。
